...
py_dlg_path = 'views.telegram.moderate.dialogs'
...
class ModerateView(BotViewInterface):
    name = 'Moderate'
    def __init__(self):
        self.description = 'Интерфейс бота для премодерации постов.'
    ...
    async def proc_start_command(self, message: Message, bot: Bot, dialog_manager: DialogManager):
        try:
            ...
            await dialog_manager.start(states.SG_Main.start, mode=StartMode.RESET_STACK, data={'user': user})
        except Exception as ex:
            try:
