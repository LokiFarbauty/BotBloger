...
py_dlg_path = 'views.telegram.vk_sync_ads.dialogs'
...
class VKSyncView_ADS(BotViewInterface):
    name = 'VKSync_ADS'
    def __init__(self):
        self.description = 'Рекламный интерфейс бота синхронизации ВКонтакте и Телеграм.'
    ...
    async def proc_start_command(self, message: Message, bot: Bot, dialog_manager: DialogManager):
        try:
            ...
            await dialog_manager.start(states.SG_VKSync_ADS.start, mode=StartMode.RESET_STACK, data={'user': user})
        except Exception as ex:
            try:
